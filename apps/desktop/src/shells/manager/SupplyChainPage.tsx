...
export function SupplyChainPage() {
  return (
    <Stack gap={16}>
      <header>
        <h1 style={{ margin: 0, fontSize: 24 }}>Supply chain hedging</h1>
        <p style={{ margin: '4px 0 0', color: 'var(--aether-fg-muted)', fontSize: 13 }}>
          Live commodity exposure for every material in your BOMs. Recommendations are
          generated by `aether-hedging` against the commodity_prices feed; accept or dismiss
          inline.
        </p>
      </header>
            ...
            <CardHeader title={r.name} subtitle={`${r.ticker} · spot ${r.spot}`}>
              <div style={{ position: 'absolute', top: 16, right: 16 }}>
                <StatusPill kind={PILL[r.action]}>
                  {r.action} {r.savings ? `· save ${r.savings}` : ''}
                </StatusPill>
              </div>
            </CardHeader>
            <CardBody>
