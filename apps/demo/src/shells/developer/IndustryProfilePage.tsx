...
const PROFILES: Record<string, IndustryEntry> = {
  'food-and-beverage': {
    ...
    capabilities: [
      {
        id: 'expired-date-tracking',
        kind: 'tracking',
        display: 'Expiry date tracking',
        description: 'Lot/batch expiry, FEFO picking, recall tracing',
      },
      {
        id: 'temperature-sensor-integration',
        kind: 'process',
        display: 'Temperature sensor integration',
        description: 'Auto-bind OPC-UA / Modbus temperature tags to material lots',
      },
      {
        id: 'cold-chain-monitor',
        kind: 'tracking',
        display: 'Cold chain monitoring',
        description: 'Continuous temperature & humidity logging with alarms',
      },
      {
        id: 'haccp-checks',
        kind: 'quality',
        display: 'HACCP control points',
        description: 'Critical control point checklists with corrective actions',
      },
      {
        id: 'recall-readiness',
        kind: 'compliance',
        display: 'Recall readiness',
        description: 'Trace any unit forwards/backwards in <2 minutes',
      },
    ],
  ...
  automotive: {
    ...
    capabilities: [
      {
        id: 'parts-serial-tracking',
        kind: 'tracking',
        display: 'Parts serial tracking',
        description: 'Per-component VIN linkage, supplier→assembly traceability',
      },
      {
        id: 'precision-calibration',
        kind: 'quality',
        display: 'Precision calibration',
        description: 'Tool wear, gauge R&R, automatic recalibration triggers',
      },
      {
        id: 'ppap-control-plan',
        kind: 'quality',
        display: 'PPAP & control plan',
        description: 'Production Part Approval Process documentation pack',
      },
      {
        id: 'andon-line-stop',
        kind: 'safety',
        display: 'Andon line-stop',
        description: 'Operator-initiated line stop tied to interlock',
      },
    ],
    ...
    capabilities: [
      {
        ...
        kind: 'compliance',
        display: 'GxP electronic records',
        description: '21 CFR Part 11 compliant signatures & audit trails',
      },
      {
        id: 'serialization-track-trace',
        kind: 'tracking',
        display: 'Serialization & track-and-trace',
        description: 'Per-unit serial numbers, parent-child aggregation',
      },
      {
        id: 'environmental-monitoring',
        kind: 'quality',
        display: 'Environmental monitoring',
        description: 'Cleanroom particle, viable, and pressure differentials',
      },
    ],
...
export function IndustryProfilePage() {
  ...
  return (
    <Stack gap={16}>
      <header>
        <h1 style={{ margin: 0, fontSize: 24 }}>{t('page.industry.title')}</h1>
        <p style={{ margin: '4px 0 0', color: 'var(--aether-fg-muted)', fontSize: 13 }}>
          {t('page.industry.intro')}
        </p>
      </header>
      ...
      <Card>
        <CardHeader
          title={`${profile.display} · ${t('page.industry.capsSuffix', {
            count: profile.capabilities.length,
          })}`}
          subtitle={t('page.industry.capsSubtitle')}
        />
        <CardBody>
          <Stack gap={8}>
            {profile.capabilities.map((c) => (
              ...
              >
                <div>
                  <StatusPill kind={KIND_PILL[c.kind]}>
                    {t(`page.industry.kind.${c.kind}`)}
                  </StatusPill>
                </div>
                <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <Card>
          <CardHeader
            title={t('page.industry.modulesTitle')}
            subtitle={t('page.industry.modulesSubtitle')}
          />
          <CardBody>
            {profile.autoModules.length === 0 ? (
              <span style={{ fontSize: 13, color: 'var(--aether-fg-muted)' }}>
                {t('page.industry.none')}
              </span>
            ) : (
              <Stack gap={4}>
        ...
        <Card>
          <CardHeader
            title={t('page.industry.standardsTitle')}
            subtitle={t('page.industry.standardsSubtitle')}
          />
          <CardBody>
